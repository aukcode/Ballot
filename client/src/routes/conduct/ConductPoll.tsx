      ...
      return (
        <div className="ml-4 mt-8">
          {questions[currentQuestion].options.map(option => (
            <p className="inline-block bg-blue-200 rounded-full px-5 py-2 text-xl m-2">
              {option}
            </p>
          ))}
        </div>
  ...
  return (
    <div className="flex justify-center mt-8">
      <div className="mx-3 w-full sm:max-w-3xl p-4">
        <p className="text-4xl border-b-2 text-gray-600">
          {pollTitle} - Pin: {pollPin}
        </p>
        {renderNavButtons()}
        <div className="mt-8 p-8 shadow rounded-lg">
          ...
          {currentQuestion >= questions.length && (
            <p className="text-center text-lg font-semibold text-gray-800">
              That's all folks! Click Finish to end the polling
            </p>
          )}
        </div>
      </div>
